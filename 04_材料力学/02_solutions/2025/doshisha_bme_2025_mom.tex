...
〔I〕では下方への移動を正にとる．〔II〕では反力は上向きを正，たわみ $v$ は上向きを正にとり，
たわみの基礎式は $EI\,\dfrac{d^2v}{dx^2}=-M(x)$ を用いる．自由体のせん断力 $Q$ は上向きを正，
曲げモーメント $M$ は時計回りを正にとり，この符号のもとで $\dfrac{dM}{dx}=Q$ が成り立つ．
...
\begin{tikzpicture}[>=Latex]
  ...
  \draw[->] (6.0,1.0) -- (7.0,1.0) node[below,fill=white,inner sep=1pt]{$x$};
  ...
  \draw[<->] (1.0,1.95) -- (2.3,1.6) node[midway,below,font=\scriptsize,fill=white,inner sep=1pt]{$L/4$};
  \draw[<->] (6.1,0.65) -- (8.3,1.25) node[pos=0.68,below,font=\scriptsize,fill=white,inner sep=1pt]{$L/2$};
...
区間 AB の軸は $x$ 方向であり，区間 BC は $y$ 方向である．荷重 $P$ は点 C（点 A から見て
$x$ 方向に $L$，$y$ 方向に $\tfrac{L}{2}$ 離れた位置）に下向き（$z$ 軸負方向）に作用する．
固定端 A には鉛直反力 $R_A$，曲げモーメント $M_A$（$y$ 軸まわり），ねじりトルク $T_A$（$x$ 軸まわり，
はりの軸まわり）が生じる．

\medskip
\noindent\textbf{(1)}\quad ベクトルを用いず，力・モーメント・トルクの 3 つのつり合いを別々に立てる．

\noindent 力のつり合い（鉛直方向）：
\[
  0 = R_A - P \qquad\therefore\quad R_A = P\ (\text{上向き}).
\]
モーメントのつり合い（$y$ 軸まわり，曲げ）：点 C の荷重 $P$ は，$x$ 方向の腕の長さ $L$ をもつので，
点 A に曲げモーメント $P\cdot L$ を生じる．これを $M_A$ がつり合わせるから
\[
  0 = M_A - P\,L \qquad\therefore\quad M_A = PL.
\]
トルクのつり合い（$x$ 軸まわり，ねじり）：点 C の荷重 $P$ は，はりの軸（$x$ 軸）から $y$ 方向に
$\tfrac{L}{2}$ 離れているので，軸まわりにねじりトルク $P\cdot\tfrac{L}{2}$ を生じる．これを $T_A$ が
つり合わせるから
\[
  0 = T_A - P\cdot\frac{L}{2} \qquad\therefore\quad T_A = \frac{PL}{2}.
\]
\[
...
\begin{tikzpicture}[>=Latex,scale=0.9]
  ...
  \draw[->,very thick,blue] (0,-1.0) -- (0,0.2);
  \node[blue,below,fill=white,inner sep=1pt] at (0,-1.05) {$R_A=P$};
  \draw[->,very thick,red] (-0.2,0) arc (180:60:0.55) node[above,fill=white,inner sep=1pt]{$M_A=PL$};
  \draw[->,very thick,teal] (0.55,0.55) arc (110:-110:0.4) node[right=2pt,fill=white,inner sep=1pt]{$T_A=\tfrac{PL}{2}$};
\end{tikzpicture}
\end{center}

\medskip
\noindent\textbf{(2)}\quad 区間 AB（$0\le x\le L$）で断面位置 $x$ をとり，断面より先（C 側）の自由体を考える．
せん断力 $Q$ は上向きを正，曲げモーメント $M$ は時計回りを正にとる．先端 C には下向きの荷重 $P$ のみが
作用する．自由体のつり合いより，曲げモーメントは腕の長さ $(L-x)$ を用いて $M(x)=P(L-x)$（時計回り）となり，
せん断力は $\dfrac{dM}{dx}=Q$ が成り立つように符号をとると $Q(x)=\dfrac{dM}{dx}=-P$ である．
ねじりトルクは軸を一定に伝わり $T(x)=P\cdot\dfrac{L}{2}$．
\[
  Q(x) = -P\ (\text{一定}),\qquad
  M(x) = P\,(L-x),\qquad
  T(x) = \frac{PL}{2}\ (\text{一定}).
\]
端点での値は $M(0)=PL$（$=M_A$），$M(L)=0$（点 B で曲げ 0）．確かに $\dfrac{dM}{dx}=-P=Q$ が成り立つ．
\[
  \ans{\;Q(x)=-P,\qquad M(x)=P(L-x)\quad(M(0)=PL,\ M(L)=0),\qquad \frac{dM}{dx}=Q\;}
\]
...
\begin{tikzpicture}[>=Latex,scale=1.0]
  ...
  \begin{scope}
    ...
    \draw[->] (0,-1.3) -- (0,0.5) node[above]{SFD};
...
\textbf{点 B のたわみ $\delta_B$}：区間 AB は固定端 A の片持ちはりで，曲げモーメントは $M(x)=P(L-x)$．
たわみ $v$ は上向きを正とし，基礎式 $EI\,v''=-M(x)=-P(L-x)$ を $v(0)=0,\ v'(0)=0$ のもとで
2 回積分すると
\[
  EI\,v'(x) = -PLx + \frac{P}{2}x^2,\qquad
  EI\,v(x) = -\frac{P}{2}Lx^2 + \frac{P}{6}x^3 .
\]
よって点 B（$x=L$）で
\[
  \ans{\;\delta_B = v(L) = -\frac{PL^3}{3EI}\quad(\text{下向きに }\tfrac{PL^3}{3EI})\;}
\]
...
\smallskip
\textbf{点 C のたわみ $\delta_C$}：たわみは上向きを正とする．C の $z$ 変位は次の 3 つの寄与（いずれも下向き）の和である．
\begin{itemize}
...
\[
  \ans{\;\delta_C = -\left(\frac{3PL^3}{8EI} + \frac{PL^3}{4GI_p}\right)\quad(\text{下向き})\;}
\]
