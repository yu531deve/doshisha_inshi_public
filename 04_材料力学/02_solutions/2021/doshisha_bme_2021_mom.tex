...
\begin{tikzpicture}[>=Latex,scale=0.95]
  ...
  \draw[->,very thick] (0,2.5) -- (0,1.4) node[midway,right,fill=white,inner sep=1pt]{$P$};
  \node[right,fill=white,inner sep=1pt] at (0.35,3.9) {\footnotesize 丸棒 1};
  \node[right,fill=white,inner sep=1pt] at (0.35,1.4) {\footnotesize 丸棒 2};
...
\begin{tikzpicture}[>=Latex]
  ...
  \node[fill=white,inner sep=1pt] at (3.3,-0.05) {C};
  ...
  \draw[<->] (6.2,0.7) -- (8.6,1.45) node[midway,above,fill=white,inner sep=1pt,font=\scriptsize]{$L$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=Latex,scale=1.0]
  ...
  % 応力点 (σx, -τ) と (σy=0, +τ)
  \fill (\c+0.6,-0.95) circle (1.2pt); \node[right,fill=white,inner sep=1pt] at (\c+0.6,-0.95) {\scriptsize $(\sigma_x,-\tau_{xy})$};
  \fill (\c-0.6,0.95) circle (1.2pt); \node[left,fill=white,inner sep=1pt] at (\c-0.6,0.95) {\scriptsize $(\sigma_y,\tau_{xy})$};
  \draw[dashed] (\c-0.6,0.95) -- (\c+0.6,-0.95);
  \node[above,fill=white,inner sep=1pt] at (\c,\r) {\scriptsize 半径 $r$};
\end{tikzpicture}
\end{center}
